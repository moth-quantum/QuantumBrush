...
if command -v javac >/dev/null && command -v jar >/dev/null; then
    ...
    mkdir -p "$BUILD_DIR"
    if javac --release 21 "$ROOT/packaging/launcher/Launcher.java" -d "$BUILD_DIR" 2>/dev/null && \
       jar --create --file "$BUILD_DIR/launcher.jar" --main-class Launcher -C "$BUILD_DIR" . 2>/dev/null; then
