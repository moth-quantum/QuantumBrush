        ...
        <div className="floating-top-header">
          ...
          <div className="center-tools">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8 }}>
              {selectedEffect && effects.length > 0 && (
                <>
                  <div style={{ width: 140 }}>
                    <UnifiedDropdown
                      options={effects.map(eff => ({ value: eff.id, label: eff.name }))}
                      value={selectedEffect.id}
                      onChange={(val) => selectEffect(effects.find(eff => eff.id === val))}
                    />
                  </div>
                  {selectedEffect.user_input && selectedEffect.user_input['Strength'] && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 8 }} title="Effect Strength">
                      <Zap size={14} style={{ color: 'var(--accent)' }} />
                      <input
                        type="range"
                        className="slider-input"
                        style={{ width: 60 }}
                        min={selectedEffect.user_input['Strength'].min}
                        max={selectedEffect.user_input['Strength'].max}
                        step="0.05"
                        value={effectParams['Strength'] ?? selectedEffect.user_input['Strength'].default}
                        onChange={(e) => setEffectParams(prev => ({ ...prev, 'Strength': parseFloat(e.target.value) }))}
                      />
                      <input
                        type="number"
                        className="form-input"
