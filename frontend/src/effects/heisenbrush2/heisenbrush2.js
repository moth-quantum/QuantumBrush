...
function scaleToRange(x, in_min = 1, in_max = 100, out_min = 2, out_max = 10) {
    if (x < in_min || x > in_max) x = Math.max(in_min, Math.min(x, in_max));
...
function runHeisenbergHardware(dt_list, radius, phi, theta, reportProgress) {
    ...
    if (circuits.length > 0) {
        console.log("Quantum Circuit [heisenbrush2] (last step):\n" + circuits[circuits.length - 1].exportQASM(""));
    }
...
export async function run(params, reportProgress) {
    ...
    let split_paths = utils.splitPathFromClicks(path, clicks);
    let normalized_distances = new Array(split_paths.length).fill(0.1);
