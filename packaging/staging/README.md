...
However, Luckily, the installer provides automatic OpenJDK + Python installation through `miniconda` thus you actually don’t need to do anything! The native installers and `setup.sh` create a dedicated Python environment at `~/.quantumbrush/env` and store all necessary libraries there. More of this is described under [Installation Instructions](https://github.com/moth-quantum/quantumbrush?tab=readme-ov-file#installation-instructions).
...
Download the latest release [here](https://github.com/moth-quantum/QuantumBrush/releases/latest).
...
| Ubuntu ISO option | `QuantumBrush-*-ubuntu-amd64.iso` | Mount the ISO and run `./install-quantumbrush.sh` |

On first launch, QuantumBrush automatically configures Java and Python for you. No terminal required.

Signed release builds avoid macOS Gatekeeper crashes. If macOS still warns you, use a signed release build from the official Releases page rather than bypassing security.
...
2. macOS/Linux: open Terminal and run `sh ~/Downloads/QuantumBrush-Installer.sh`
3. Windows: double-click `QuantumBrush-Installer.bat`
4. When prompted, press `y` to run setup and install Java/Python automatically.
5. Launch with `./RunQuantumBrush.sh`, `RunQuantumBrush.command`, or `RunQuantumBrush.bat`.

## Update an existing install

1. Open a terminal in your `QuantumBrush` folder.
2. Run `sh ~/QuantumBrush/update.sh` on macOS/Linux, or `bash update.sh` on Windows with Git Bash.
