...
handle_existing() {
    ...
    if find "$INSTALL_DIR" -maxdepth 1 -name "QuantumBrush*.jar" -type f 2>/dev/null | grep -q .; then
        ...
        echo "  3) Cancel"
        echo
        read -p "Choose (1-3): " -n 1 -r; echo

        case "$REPLY" in
            1) info "Updating existing installation..." ;;
            2)
                read -rp "Enter new directory: " new_dir
        ...
        read -p "Use this directory anyway? (y/N): " -n 1 -r; echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            read -rp "Enter a different directory: " new_dir
            [ -z "$new_dir" ] && { err "No directory specified."; exit 1; }
            INSTALL_DIR="$new_dir"
            handle_existing
        fi
...
do_install() {
    ...
    chmod +x "$INSTALL_DIR/setup.sh" "$INSTALL_DIR/update.sh" \
             "$INSTALL_DIR/RunQuantumBrush.sh" \
...
main() {
    ...
    if [ -f "$INSTALL_DIR/setup.sh" ]; then
        echo
        read -p "Run setup now to install Java and Python dependencies? (Y/n): " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
    ...
    printf "  cd %s && ./RunQuantumBrush.sh\n\n" "$INSTALL_DIR"
}
