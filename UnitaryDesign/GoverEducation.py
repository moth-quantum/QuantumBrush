...
class GroverGame:
    def __init__(self):
        ...
        self.raw_amplitudes = []  # Add this line

        # Add size buttons (in __init__ method)
        self.size_buttons = [
            Button(WIDTH-250, 80, 60, 30, "4", LIGHT_BLUE),
            Button(WIDTH-180, 80, 60, 30, "8", LIGHT_BLUE),
            Button(WIDTH-110, 80, 60, 30, "16", LIGHT_BLUE)
        ]

        # Quantum components
        self.n_qubits = int(math.log2(self.database_size))
        self.qc = None
        self.oracle = None
        self.diffusion = None
    ...
    def handle_simulation_events(self, event, mouse_pos):
        # Add to existing method before simulation buttons handling
        self.back_button.check_hover(mouse_pos)
        if self.back_button.is_clicked(mouse_pos, event):
           self.state = MENU
           self.auto_run = False
           return  # Exit early to prevent other button interactions
            # Handle database size buttons
        size_changed = False
        for button in self.size_buttons:
           button.check_hover(mouse_pos)
           if button.is_clicked(mouse_pos, event):
              new_size = int(button.text)
              if new_size != self.database_size:
                 self.database_size = new_size
                 self.n_qubits = int(math.log2(self.database_size))
                 self.marked_item = random.randint(0, self.database_size-1)
                 self.reset_simulation(keep_marked=False)
                 size_changed = True
                 break  # Only process one size change per click

        # Only process other buttons if no size change occurred
        if not size_changed:
         ...
         for i, button in enumerate(self.simulation_buttons):
            ...
            if button.is_clicked(mouse_pos, event):
                if i == 0:  # Step Forward
                    ...
                    self.auto_run = not self.auto_run
                    self.auto_run_timer = pygame.time.get_ticks()
                    self.simulation_buttons[3].text = "Stop Auto" if self.auto_run else "Auto Run"
                elif i == 4:  # Faster
                    self.auto_run_speed = min(5.0, self.auto_run_speed * 1.5)
                elif i == 5:  # Slower
                    self.auto_run_speed = max(0.5, self.auto_run_speed / 1.5)
                break  # Only process one button click per event
    ...
    def draw_simulation(self):
        ...
        for i, text in enumerate(info_text):
            ...
            # Add database size selection buttons
        size_label = FONT.render("Database Size:", True, BLACK)
        self.screen.blit(size_label, (WIDTH - 300, 85))
        for button in self.size_buttons:
        # Highlight selected size
            if int(button.text) == self.database_size:
               button.color = ORANGE
            else:
    ...
    def draw_database(self):
        ...
        for i in range(self.database_size):
            ...
            if i == self.marked_item:
                color = RED if amplitude < 0 else GREEN
            else: 
                 color = BLUE if amplitude < 0 else LIGHT_BLUE
    ...
    # Generate new marked item when requested
       if not keep_marked:
          self.marked_item = random.randint(0, self.database_size - 1)
        # Verify database size is valid
       if not math.log2(self.database_size).is_integer():
            self.database_size = 8  # Fallback to default
            
       self.n_qubits = int(math.log2(self.database_size))

    # Rebuild quantum components with current marked item
       self.n_qubits = int(math.log2(self.database_size))
       self.qc = QuantumCircuit(self.n_qubits)
       self.qc.h(range(self.n_qubits))  # Initial superposition

    # Rebuild oracle with current marked item
       diag = [1]*(2**self.n_qubits)
       diag[self.marked_item] = -1
       self.oracle = Diagonal(diag)
       self.oracle.label = "Oracle"

    # Rebuild diffusion operator
       diffusion_qc = QuantumCircuit(self.n_qubits, name="Diffusion")
       diffusion_qc.h(range(self.n_qubits))
       diffusion_qc.x(range(self.n_qubits))
       diffusion_qc.h(self.n_qubits-1)
       diffusion_qc.mcx(list(range(self.n_qubits-1)), self.n_qubits-1)
       diffusion_qc.h(self.n_qubits-1)
       diffusion_qc.x(range(self.n_qubits))
       diffusion_qc.h(range(self.n_qubits))
       self.diffusion = diffusion_qc.to_gate()
    ...
    def update_amplitudes(self):
        # Get current statevector directly from circuit
        statevector = Statevector(self.qc)
        self.amplitudes = [amp.real for amp in statevector.data] 
    ...
    def step_simulation(self):
        ...
        if self.current_step % 2 == 0:
            self.qc.append(self.oracle, range(self.n_qubits))
        else:
            self.qc.append(self.diffusion, range(self.n_qubits))
